...
/* ---------------------------------------------------------
   6. FINALE — lines settle in sequence, then arm the burst button
   --------------------------------------------------------- */
function startFinale() {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        lines.forEach((line, i) => setTimeout(() => line.classList.add('settled'), i * 260));
        observer.disconnect();
      }
    });
  }, { threshold: 0.4 });
  ...
  initFinaleBurst(document.getElementById('finaleBtn'), document.getElementById('burstCanvas'));
}
